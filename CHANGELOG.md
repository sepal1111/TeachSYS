...
- **修改模組/檔案**：
  - 前端業務邏輯：`static/js/point-cards.js`
- **修改類別**：UI 顯示格式調整
- **修改內容**：
  1. **最常使用卡片排行**：簡化為僅顯示「卡片序列號（分數分）」，移除原本的卡片名稱標籤文字，聚焦於教師辨識實體卡片所需的序列號資訊。
  2. **學生刷卡獲得總分排行**：欄位順序調整為「姓名 座號號」（如：王小明 1號），並統一總分數字不加空格（如：+15分），與最常使用卡片排行之數字顯示風格一致。

## [2026-09-14f] 修正點數卡使用統計分析彈窗欄位缺漏與 undefined 顯示問題

- **修改模組/檔案**：
  - 後端點數卡路由：`src/routes/pointCards.ts`
  - 前端業務邏輯：`static/js/point-cards.js`
- **修改類別**：錯誤修正（Bug Fix）
- **背景與需求**：
  - 使用者回報「點數卡使用統計分析」彈窗中，「學生刷卡獲得總分排行」欄位顯示為 `座號 undefined - 王小明` 與 `undefined 分`；「最常使用卡片排行」則只顯示卡片名稱與分數，未列出卡片序列號，不利於教師辨識具體是哪一張實體卡片。
- **修改內容**：
  1. **修正 `GET /api/point-cards/:courseId/stats` 回傳欄位命名不一致問題**：
     - 後端原本回傳 `number`／`totalScore`（駝峰式）與 `total_redemptions`，但前端讀取的是 `seat_number`／`total_score`／`total_redeemed`（蛇形式），命名不一致導致對應欄位讀取為 `undefined`。統一改為蛇形式命名，與其餘 API 回傳格式一致。
  2. **最常使用卡片排行新增卡片序列號**：
     - 後端統計結果新增回傳 `card_no`（卡片序列號），前端排行列表同步顯示序列號，方便教師對照實體卡片。
